...
FILE=top
FILES="CPU.v SEQUENCER.v PROGRAMCOUNTER.v RAM.v IRDECODER.v OPRDECODER.v MULTILATCH.v ADDAND.v CLORIN.v INCREMENTER.v ROTATER.v"
...
echo --LINTING
#$DR antonkrug/verilator-slim /bin/sh -c "verilator -Wall -DNOTOP --top-module top --lint-only top.v $FILES"
#if [ $? != 0 ]; then exit 1; fi
...
if [ -f tb/CPU_tb.v ]; then 
    echo --TESTING
    $DR cranphin/iverilog iverilog -o CPU.vvp tb/CPU_tb.v $FILES 
    if [ $? != 0 ]; then exit 1; fi
